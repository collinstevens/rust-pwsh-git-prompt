...
$childArguments = @('--no-pager', 'branch', '--show-current')
...
[pscustomobject]@{
    Benchmark = 'git-current-branch'
    Metric = 'Parent timestamp immediately before Process.Start until WaitForExit returns; includes Git startup, current branch lookup, redirected output, and shutdown.'
